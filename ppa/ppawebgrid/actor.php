...
/*	$sql = "" .
	...
	$sql = "" .
		"SELECT slot_chapter.slot, movie.actors ". 
		"FROM movie ".
		"LEFT JOIN chapter ON chapter.movie = movie.id ".
		"LEFT JOIN slot_chapter ON slot_chapter.chapter = chapter.id ".
		"WHERE movie.actors LIKE  '%" . $_GET['actor'] . "%' ";
...
/*	$sql = "" .
	...
	$sql = "" .
		"SELECT slot_chapter.slot, serie.starring ". 
		"FROM serie ".
		"LEFT JOIN chapter ON chapter.serie = serie.id ".
		"LEFT JOIN slot_chapter ON slot_chapter.chapter = chapter.id ".
		"WHERE serie.starring LIKE  '%" . $_GET['actor'] . "%' ";
...
/*	$sql = "" .
	...
	$sql = "" .
		"SELECT slot_chapter.slot, special.starring ". 
		"FROM special ".
		"LEFT JOIN chapter ON chapter.special = special.id ".
		"LEFT JOIN slot_chapter ON slot_chapter.chapter = chapter.id ".
		"WHERE special.starring LIKE  '%" . $_GET['actor'] . "%' ";
...
?>
      <TR height="45">
  			...
  			<img src="<?=( $row['logo'] != "" ? "http://" . $URL_PPA . "/ppa/ppa/" . $row['logo'] : "images/empty.gif" )?>"/><div><?= $row['name'] ."<br>". $row['number'] ?></div></TD>
