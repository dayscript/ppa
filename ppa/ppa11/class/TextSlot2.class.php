...
Class TextSlot
{
	...
	function processType2()
	{
		...
		{
			if( preg_match( '/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/', $tmp[$i] ) )
				$date[$i] = $tmp[$i];
			else
				echo "error";
//				throw new Exception('Formato de fecha incorrecto. Columna ' . $i);
		}
		$this->file = array_slice( $this->file, 1 );
...
//					$this->result[$j]['time']  = strtotime( trim( $date[$i] ) . " " . trim( $tmp[0] ) );
					echo $i . " " . $date[$i] . " " . $tmp[0] . "<br>";
					$this->result[$j]['title'] = trim( $tmp[$i] );
					$j++;
				}
			}
		}exit;
	...
	function processType3()
	{
		...
		foreach( $this->file as $reg )
		{
				...
				trim( $tmp[2] ) != "" )
			{
				...
				$this->result[$i]['title'] = trim( $tmp[2] );
				$i++;
			}
		}
		unset($this->file);
