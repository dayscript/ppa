...
//  $sql = "SELECT  c.id, c.name FROM channel c, slot s WHERE c.id = s.channel AND s.date =  '".$_POST['ano']."-".$_POST['mes']."-".$numdays."' AND s.title != '' GROUP  BY c.id";
  $sql = "SELECT  c.id, c.name, c.description FROM channel c, slot s WHERE c.id = s.channel AND s.date =  '".$_POST['ano']."-".$_POST['mes']."-".$numdays."' AND s.title != '' GROUP  BY c.id ORDER BY c.name";
  $query = db_query( $sql );
?>
