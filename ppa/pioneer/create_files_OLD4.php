...
if( !isset( $argv[1] ) )
    die( "Se debe especificar el NOMBRE la cabecera ( create_files.php NOMBRE )\n\n");
...
$nodesc    = ( ( isset( $argv[3] ) && $argv[3] == "1" ) || ( isset( $argv[2] ) && $argv[2] == "1" ) );
$init_date = ( ( isset( $argv[2] ) && preg_match( "/^[0-9]{4}\\-[0-9]{2}\\-[0-9]{2}$/", $argv[2] ) ) ? strtotime( $argv[2] ) : ( ( isset( $argv[3] ) && preg_match( "/^[0-9]{4}\\-[0-9]{2}\\-[0-9]{2}$/", $argv[3] ) ? strtotime( $argv[3] ) : time( ) ) ) );

PRINTLN( date("[H:i:s] ") . "Generando archivo para " . $argv[1] . " " . date( "Y-m-d", $init_date ) );
...
$start_date = date("Y-m-d", $init_date - $DAY );
$stop_date  = date("Y-m-d", $init_date + ($DAYS * $DAY) );
$stop_date_ppv  = date("Y-m-d", $init_date + (30 * $DAY) );
...
println( date("[H:i:s] ") . "Generando archivo de categorías " . $PREFIX . date("md", $init_date ) .".cat" );
$file       = new WriteFile( $PREFIX . date("md", $init_date ) .".cat" );
...
println( date("[H:i:s] ") . "Generando archivo de Canales " . $PREFIX . date("md", $init_date) .".chn" );
$file = new WriteFile( $PREFIX . date("md", $init_date) .".chn" );
	...
	if( $argv[2] == "PPV_ALL" )	{
		...
		if( $copies )	{
			...
			foreach( $copies as $CopyName => $CopyObj )	{
				$file->write( $CopyObj['id'], 10, "," );			//     1.    10  CHANNEL REFERENCE NUMBER (Link to Schedule File)
				$file->write( isset( $chn_alias [ $CopyName ] ) ? $chn_alias [$CopyName] : $CopyName, 6, "," );      //     2.     6  CALL LETTER/CABLE SERVICE NAME
				$file->write( $row['name'], 30, "," );          //     3.    30  EXPANDED CALL LETTERS
				$file->write( ""/*$row['afiliation']*/, 2, "," );     //     4.     2  AFFILIATION FIELD
				$file->write( ""/*$row['description']*/, 1, "," );    //     5.     1  CHANNEL DESCRIPTION
				$file->write( "", 3, "," );                     //     6.     3  BROADCAST CHANNEL NUMBER 3  BROADCAST CHANNEL NUMBER
...
println( date("[H:i:s] ") . "Generando archivo de Programas " . $PREFIX . date("md", $init_date) .".prg" );

$prg = new Program();
$sch = new Schedule();

$file     = new WriteFile( $PREFIX . date("md", $init_date) .".prg" );
$slot_id  = Array();
$channels = Array();
...
$sql = "SELECT ".
       ...
       "  channel.id id_chn, channel.name name_chn, channel.shortname ".
...
while($row = db_fetch_array($result))
{
	...
	if( $prg->appendReg( $row['id_chp'], $row['title_slt'] ) )
	{
		...
		if( tvCablePatch2($ID_CLIENT, $row['id_chn']) )
		{
			...
			$row['title'] = $row['name_chn'];
			$row['title_slt'] = $row['name_chn'];
			$row['spanishtitle'] = $row['name_chn'];
		}
...
println( date("[H:i:s] ") . "Total series: $cont");
...
println( date("[H:i:s] ") . "Total películas: $cont");
...
println( date("[H:i:s] ") . "Total especiales: $cont");
...
$sql = "SELECT ".
       ...
       "  client_channel._group, channel.shortname ".
...
println( date("[H:i:s] ") . "Total programación sin clasificar: $cont" );
...
println ( date("[H:i:s] ") . "Total Canales sin programación: $cont") ;
/************************
* *
...
println ( date("[H:i:s] ") . "Generando archivo de horarios " . $PREFIX . date("md", $init_date) .".sch" );

$file     = new WriteFile( $PREFIX . date("md", $init_date) .".sch" );
$channels = Array();
...
println ( date("[H:i:s] ") . "Total archivo de horarios: $cont" );
unset($last_array);
...
$file = new WriteFile( $PREFIX . date("md",$init_date) .".prg", "a+" );
...
if(!empty ($channels) )
{
	...
	println ( date("[H:i:s] ") . "Total canales: " . count( $channels ) );
...
println( date("[H:i:s] ") . "total Actualizado archivo de programas: $cont");
...
$file = new WriteFile( $PREFIX . date("md",$init_date) .".ppv" );
...
while($next_row = db_fetch_array($result))
{
	...
	if( $lastID != $row['id'] )   {
		foreach( $buffer as $key => $bdatas )	{
			foreach( $bdatas as $bdata )	{
				$file->write( $bdata[0], 10, ",", "" ); 		//"IpgProviderId" - Service identifier; same as in *.sch and *.chn file
				$file->write( $bdata[1], 10, ",", "" );         //"EventID"       - Unique ID that identifies an event in a time slot on a
				$file->write( $bdata[2], 10, "," );        		//"CallLetters"   - Same as call letters of the PPV service in *.chn file.
				$file->write( $bdata[3], 20, ",", "" );         //"StartDateTime" - Start time of event same as in *.sch file.  In UTC time.
				$file->write( $bdata[4], 10, ",", "" );         //"RunningTime"   - Time slot duration in minutes.  Note that this may be
				$file->write( $bdata[5], 100, "," );            //"Title" - Title as in the *.prg file.
				$file->write( $bdata[6], 100, "", "" );         //"Cost"  - Cost of event in centavos.
				$file->writeLn( );
			}
		}
		$buffer = array();
...
//	$cost_field = $row['number'] <= 113 ? "620" : "1110";
	...
	if( $argv[2] == "PPV_ALL" )	{
		...
		if( $copies )	{
			$copies = json_decode( $copies, true );
			foreach( $copies as $CopyName => $CopyObj )	{
				if( !isset( $buffer[$CopyObj['id']] ) )
					$buffer[$CopyObj['id']] = array();
				$bdata = array();
				$bdata[] = $CopyObj['id'];
				$bdata[] = $row['sid'];
				$bdata[] = $CopyName;
...
println( date("[H:i:s] ") . "Comprimiendo archivos" );

@unlink(OUT_PATH . $PREFIX . ".zip");

$command = "cd " . OUT_PATH . "; zip " . $PREFIX . ".zip " . $PREFIX . date("md",$init_date) .".*";
`$command`;

println( $command );
println( date("[H:i:s] ") . "Enviando archivo" );

$command = "scp " . OUT_PATH . $PREFIX . ".zip " . $props->getProperty( "ftp_user" ) ."@" . $props->getProperty( "ftp_server" ) . ":~/" . $PREFIX . date("md",$init_date) .".zip";
`$command`;
