...
$PREFIX    = $props->getProperty("prefixV36");
...
while($row = db_fetch_array($result))
{
	...
	if( $prg->appendReg( $row['id_chp'], $row['title_slt'] ) )
	{
		...
		if( tvCablePatch2($ID_CLIENT, $row['id_chn']) )
		{
			...
			$row['title'] = "Programación para adultos";
			$row['title_slt'] = "Programación para adultos";
			$row['spanishtitle'] = "Programación para adultos";
		}
		...
		$file->write( "", 440, "," );                             //     28.  440  DESCRIPTION 1
		if($nodesc)
			$file->write( "", 440, "," );                            //     29.  440  DESCRIPTION 2                                               
		else
			$file->write( $row['ldesc'], 440, "," );                            //     29.  440  DESCRIPTION 2                                               
		$file->write( "", 440, "," );                                        //     30.  440  DESCRIPTION 3                                               
		...
		$file->write( "", 14 );                                             //     38.   14  SERIES/EPISODE ID FOR V3.5+ CLIENTS
		$file->writeLn( );
	}
}
...
while($row = db_fetch_array($result))
{
	...
	if( $prg->appendReg( $row['id_chp'], $row['title'] ) )
	{
		...
		if($nodesc)
			...
			$file->write( $desc, 440, "," );                                    //     29.  440  DESCRIPTION 2                                               
		$file->write( "", 440, "," );                                        //     30.  440  DESCRIPTION 3                                               
...
//	for($date = strtotime($start_date); $date <= strtotime($stop_date); $date += (24*60*60))
	for($c = 10000, $date = strtotime($start_date); $date <= strtotime($stop_date); $date += (60*60), $c ++)
	{
		$sch->appendReg(
		                 $row['id'],
		                 $row['id'] . $c,
    ...
    $file->write( $row['id'] . $c, 10, "," );             //     1.    10  PROGRAM ID NUMBER (Link to Schedule File)                   
    ...
    $file->write( "", 1, "," );                               //     17.    1  QUALITY STARS ON MOVIES                                     
    $file->write( "", 25, "," );                              //     18.   25  COUNTRY OF ORIGIN ON MOVIES                                 
    $file->write( 60, 4, "," );                            //     19.    4  TRUE DURATION (IN MINUTES)                                  
    $file->write( "", 40, "," );                              //     20.   40  DIRECTOR ON MOVIES                                          
    $file->write( "", 36, "," );                              //     21.   36  ACTOR 1 ON  MOVIES                                          
    $file->write( "", 36, "," );                              //     22.   36  ACTOR 2  "    "                                             
    $file->write( "", 36, "," );                              //     23.   36  ACTOR 3  "    "                                             
    ...
    if($nodesc)
      ...
      $file->write( $row['name'], 440, "," );                   //     29.  440  DESCRIPTION 2                                               
    $file->write( "", 440, "," );                              //     30.  440  DESCRIPTION 3                                               
...
while($sch->fetchScheduleArray())
{
	...
	/**
	 * @todo 
	 * An encoded string identifying the series and episode of a show, to assist with DVR series 
	 * recordings. This is a 14 character field, starting with a 2 character type (MV, EP, SH, SP) == 
	 * (Movie, Series Episode, Show, Sports) followed by an 8 character program ID or series ID 
	 * followed by a 4 character Episode ID. For movies, the episode ID is 0000. For generic episodes 
	 * of a series, the episode ID is 0000 (and the program type is SH instead of EP) but the series ID is 
	 * the same.
	 */
	$file->write( "", 14 );                                                                        //     11.   14  SERIES/EPISODE ID FOR V3.5+ CLIENTS                                         
	$file->writeLn( );
...
if(!empty ($channels) )
{
	...
	while($row = db_fetch_array($result))
	{
		...
		$file->write( "", 128, "," );                             //     5.   128  ALT. TITLE 3                                                
		$file->write( "", 128, "," );                             //     6.   128  ALT. TITLE 4                                                
		$file->write( "", 128, "," );                             //     7.   128  ALTERNATE TITLE 5 (shortest of above)
		$file->write( "", 128, "," );                             //     8.   128  SUBTITLE                                                    
		$file->write( "", 128, "," );                             //     9.   128  ALTERNATE SUBTITLE 1                                        
		$file->write( "", 128, "," );                             //     10.  128  ALTERNATE SUBTITLE 2                                        
		$file->write( "", 70, "," );                              //     11.   70  EPISODE TITLE                                               
		$file->write( "62", 2, "," );                             //     12.    2  PROGRAM TYPE (Other = 62)
		$file->write( "", 18, "," );                              //     13.   18  CATEGORY   - numeric code, refer to .CAT file               
		$file->write( "", 2, "," );                               //     14.    2  MPAA RATING                                                 
		$file->write( "", 2, "," );                               //     15.    2  PARENTAL RATING  (VCHIP)                                    
		$file->write( "", 4, "," );                               //     16.    4  YEAR OF RELEASE ON MOVIES                                   
...
$file->write( "IpgProviderId", 10, "," );	    //"IpgProviderId" - Service identifier; same as in *.sch and *.chn file     
$file->write( "EventID", 10, "," );             //"EventID"       - Unique ID that identifies an event in a time slot on a service
$file->write( "CallLetters", 10, "," );         //"CallLetters"   - Same as call letters (in double quotes) of the PPV service in *.chn file.
$file->write( "StartDateTime", 19, "," );       //"StartDateTime" - Start time of event same as in *.sch file in UTC time in format YYYY-MM-DD HH:MM:SS.
$file->write( "RunT", 4, "," );    				//"RunningTime"   - Time slot duration in total minutes. Note that this may be longer than actual duration of event in the *.prg file.
$file->write( "Title", 128, "," );              //"Title" - Title (in double quotes) as in the *.prg file.
$file->write( "Cost", 10, "" );                 //"Cost"  - Cost of event in cents or in the local currency fractional unit.
$file->writeLn( );
...
while($next_row = db_fetch_array($result))
{
	...
	$file->write( $time_field, 19, ",", "" );           //"StartDateTime" - Start time of event same as in *.sch file.  In UTC time.
	$file->write( $duration, 4, ",", "" );              //"RunningTime"   - Time slot duration in minutes.  Note that this may be   
	$file->write( $row['title'], 128, "," );            //"Title" - Title as in the *.prg file.                                     
	$file->write( $cost_field, 10, "", "" );            //"Cost"  - Cost of event in centavos.                                      
	$file->writeLn( );
	$cid[$row['id']] = $row['id'];
...
while($row = db_fetch_array($result))
{
	for($date = strtotime($start_date), $i=0; $date <= strtotime($stop_date_ppv); $date += (60*60), $i++)
	{
			...
			$file->write( $row['shortname'], 10, "," );           //"CallLetters"   - Same as call letters of the PPV service in *.chn file.  
			$file->write( $time_field, 19, ",", "" );             //"StartDateTime" - Start time of event same as in *.sch file.  In UTC time.
			$file->write( "60", 4, ",", "" );                     //"RunningTime"   - Time slot duration in minutes.  Note that this may be   
			$file->write( "Programación " . $row['name'], 128, "," );                //"Title" - Title as in the *.prg file.                                     
			$file->write( $cost_field, 10, "", "" );              //"Cost"  - Cost of event in centavos.                                      
			$file->writeLn( );
	}
}
