...
$sql = "SELECT channel.id chid, name, chapter.id cid, date, time, slot.title, chapter.description, movie.gender " .
"FROM slot, slot_chapter, chapter, channel, movie " .
"WHERE " . 
"date BETWEEN '2009-04-20' AND '2009-04-27' AND " . 
...
$rtv = $dom->createElement("RiGHTvEPG");
//$rtv->setAttribute("xmlns", "http://www.orca.tv/RiGHTv/EPG");
//$rtv->setAttribute("xmlns:xsi","http://www.w3.org/2001/XMLSchema-instance");
//$rtv->setAttribute("xsi:schemaLocation","http://www.orca.tv/RiGHTv/EPG RiGHTvEPG.xsd");
$dom->appendChild($rtv);

//<ChannelsList>
$chnl = $dom->createElement("ChannelsList");
$rtv->appendChild($chnl);
//channel
$channelName = "";
while($row = db_fetch_array($result))
{
//<channel>
	if($row["name"]!=$channelName)
	{
		$chn = $dom->createElement("channel");
		$chn->setAttribute("id",$row["chid"]);
		$channelName=$row["name"];
	}	
		$prg = $dom->createElement("program");
		$prg->setAttribute("id", $row["cid"]);
		$prg->setAttribute("time", $row["date"] . "T" . $row["time"] . "-05:00");
		$prg->setAttribute("genre", utf8_encode($row["gender"]));
		$chn->appendChild($prg);

		$tit = $dom->createElement("title", utf8_encode($row["title"]));
		$prg->appendChild($tit);

		$des = $dom->createElement("description", utf8_encode($row["description"]));
		$prg->appendChild($des);

		if($prg->previousSibling)
		{
//			print_r( date("Y-m-d H:i:s",strtotime($prg->previousSibling->getAttribute("time"))) );
			$prv = strtotime($prg->previousSibling->getAttribute("time"));
			$cur = strtotime($prg->getAttribute("time"));
			$dur = ($cur - $prv)/60;
			$prg->previousSibling->setAttribute("duration", "PT" . floor($dur/60) . "H" . ($dur%60) . "M" );
		}
		$chn->appendChild($prg);
//	}
/*	else
	...
	$chnl->appendChild($chn);
	
//	$prg = $dom->createElement("program");
}
$prg->setAttribute("duration", "1H00M");
...
$stop_date  = date("Y-m-d", time() + (8 * $DAY) );
...
$sql = "SELECT channel.id, channel.shortname, channel.name, ". 
...
$sql = "SELECT ".
       "  slot.id id_slt, slot.date, slot.time, slot.duration, slot.title title_slt, ".
...
$sql = "SELECT ".
       ...
       "  AND channel.id = slot.channel ".
       "  AND slot.date BETWEEN '". $start_date ."' AND '". $stop_date ."' ".
       "  AND slot.id NOT IN (" . implode(",", $slot_id ) . ") ".
       "ORDER by client_channel.number, slot.date, slot.time ";
...
while($row = db_fetch_array($result))
{
	...
	if( $prg->appendReg( $row['id_chp'], $row['title'] ) )
	{
		...
		if( tvCablePatch($ID_CLIENT, $row['id_chn']) )
		{
			$desc = "Precio sin IVA - " . $row['title'];
		}
		else if( interCablePatch($ID_CLIENT, $row['id_chn']) )
		{
			$cost = $props->getProperty( "channel_" . $row['id_chn'] );
			$desc = "Bs.F. " . $cost . ",10 con IVA - " . $row['title'];
			unset($cost);
		}
...
$sql = "SELECT ".
       ...
       "  AND ppa.client.id = " . $ID_CLIENT ."".
       "  AND client_channel._group like 'PPV' ".
       "  AND channel.id NOT IN (" . implode(",", $cid ) . ") ".
       "ORDER by client_channel.number ";
...
/*$ftp = new FTP($props->getProperty( "ftp_server" ), $props->getProperty( "ftp_user" ),$props->getProperty( "ftp_passwd" ));
$ftp->open();
$ftp->put( OUT_PATH . $PREFIX . ".zip", $PREFIX . date("md") .".zip");
$ftp->close();*/
