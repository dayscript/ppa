...
$stop_date  = date("Y-m-d", time() + (8 * 24 * 60 * 60) );
//$stop_date  = date("Y-m-d", time() + (2 * 24 * 60 * 60) );
...
$sql = "SELECT ".
       "  slot.id id_slt, slot.date, slot.time, slot.duration, ".
...
while($row = db_fetch_array($result))
{
	...
	if( $prg->appendReg( $row['id_chp'], $row['title'] ) )
	{
		...
		$file->write( $row['id_chp'], 10, "," );						    //     1.    10  PROGRAM ID NUMBER (Link to Schedule File)                   
		$file->write( $row['title'], 128, "," );                            //     2.   128  TITLE                                                       
		$file->write( $row['spanishtitle'], 128, "," );                     //     3.   128  ALT. TITLE 1                                                
		$file->write( $row['alt2'], 128, "," );                             //     4.   128  ALT. TITLE 2                                                
		...
		$file->write( "33", 2, "," );                                       //     12.    2  PROGRAM TYPE                                                
		...
		$file->write( $row['sdesc'], 80, "," );                             //     28.  440  DESCRIPTION 1                                               
		$file->write( $row['ldesc'], 140, "," );                            //     29.  440  DESCRIPTION 2                                               
...
$sql = "SELECT ".
       ...
       "FROM program.client, channel, client_channel, slot ".
       "WHERE program.client.id = client_channel.client ".
       "  AND channel.id = client_channel.channel ".
       "  AND program.client.id = " . $ID_CLIENT ."".
       "  AND channel.id = slot.channel ".
       "  AND slot.date BETWEEN '". $start_date ."' AND '". $stop_date ."' ".
...
while($row = db_fetch_array($result))
{
	...
	if( $prg->appendReg( $row['id_chp'], $row['title'] ) )
	{
		...
		$file->write( "", 80, "," );                                        //     28.  440  DESCRIPTION 1                                               
		$file->write( $desc, 140, "," );                                    //     29.  440  DESCRIPTION 2                                               
		$file->write( "", 40, "," );                                        //     30.  440  DESCRIPTION 3                                               
...
while($next_row = db_fetch_array($result))
{
	...
	if( $row['id'] == 506 && ereg( "00:00:00|19:00:00", $row['time'] ) ) $cost_field = "1210";
	elseif( $row['id'] == 507 && ereg( "00:00:00|19:00:00", $row['time'] ) ) $cost_field = "1010";
	elseif( $row['id'] == 508 && ereg( "00:00:00|19:00:00", $row['time'] ) ) $cost_field = "1310";
...
$sql = "SELECT ".
       ...
       "  client_channel.number ".
       "FROM program.client, channel, client_channel ".
       "WHERE program.client.id = client_channel.client ".
       "  AND channel.id = client_channel.channel ".
       "  AND program.client.id = " . $ID_CLIENT ."".
       "  AND client_channel._group like 'PPV' ".
...
$ftp = new FTP("localhost", "pioneer","pi0505");
$ftp->open();
