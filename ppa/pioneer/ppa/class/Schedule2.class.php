...
Class Schedule
{
	...
	function Schedule()
	{
		...
		$sql = "CREATE TEMPORARY TABLE schedule ".                  
		       ...
		       "    new_episode tinyint(1), ".   //     5.     1  NEW EPISODE FLAG                     
	...
	{
		if($this->new_episode==1) return true;
		else return false;
	}
	...
	function appendReg($channel, $program, $title, $date, $time, $duration, $new_episode)
	{
	...
	function fetchScheduleArray()
	{
		if( $row = db_fetch_array( $this->result ) )
		{
			...
			$this->new_episode= $row['new_episode'];
			return true;
		}
		else
		{
