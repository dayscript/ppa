...
$sql = "SELECT channel.id, channel.shortname, channel.name, channel.review description, client_channel.number ".
		"FROM channel, client_channel ".
		"WHERE channel.id = client_channel.channel AND client_channel.client = " . $ID_CLIENT ." ".
		"ORDER BY client_channel.number, channel.shortname";
	...
	if( $argv[2] == "PPV_ALL" )	{
		...
		if( $copies )	{
			...
			foreach( $copies as $CopyName => $CopyObj )	{
				...
				$file->write( $row['name'] . substr( $CopyName, -1 ) , 30, "," );          //     3.    30  EXPANDED CALL LETTERS
				$file->write( ""/*$row['afiliation']*/, 2, "," );     //     4.     2  AFFILIATION FIELD
				$file->write( ""/*$row['description']*/, 1, "," );    //     5.     1  CHANNEL DESCRIPTION
				$file->write( "", 3, "," );                     //     6.     3  BROADCAST CHANNEL NUMBER 3  BROADCAST CHANNEL NUMBER
				$file->write( ""/*$row['timezone']*/, 1, "," );       //     7.     1  TIMEZONE
	...
	if( $argv[2] == "PPV_ALL" )	{
		...
		if( $copies )	{
			...
			foreach( $copies as $CopyName => $CopyObj )	{
				...
				$sch->appendReg(
					$row['id_chn'],
						$row['id_chp'] . "0" . substr( $CopyName, -1 ),
					$row['title_slt'],
					$row['date'],
					$row['time'],
					$row['duration'],
				...
				if( $prg->appendReg( $row['id_chp'] . "0" . substr( $CopyName, -1 ), $row['title_slt'] ) )
				{
					$cont ++;
					$actor = array();
					$row['year'] = "";
	...
	if( $argv[2] == "PPV_ALL" )	{
		...
		if( $copies )	{
			...
			foreach( $copies as $CopyName => $CopyObj )	{
				...
				$sch->appendReg(
					$row['id_chn'],
						$row['id_slt'] . "0" . substr( $CopyName, -1 ),
					$row['title'],
					$row['date'],
					$row['time'],
					$row['duration'],
...
$sql = "SELECT ".
		...
		"ORDER by client_channel.number, channel.shortname, slot.date, slot.time ";
...
{
    if( tvCablePatch2( $ID_CLIENT, $row['id'] ) )
        $row['title'] = $row['name'];
...
$sql = "SELECT ".
		"channel.id id, channel.shortname, channel.name, client_channel.number ".
