...
if(isset($_GET['search_special']))$_POST['search_special'] = $_GET['search_special'];
if(isset($_GET['search_specialEs']))$_POST['search_specialEs'] = $_GET['search_specialEs'];

if(isset($_POST['search_special'])){
	$search_special = $_POST['search_special'];
	session_register('search_special');
	$_SESSION['search_special'] = $search_special;
	session_unregister('search2_special');
	unset($_SESSION['search2_special']);
	session_unregister('search_specialEs');
	unset($_SESSION['search_specialEs']);
} else if(isset($_POST['search2_special'])){
	$search2_special = $_POST['search2_special'];
	session_register('search2_special');
	$_SESSION['search2_special'] = $search2_special;
	session_unregister('search_special');
	unset($_SESSION['search_special']);
	session_unregister('search_specialEs');
	unset($_SESSION['search_specialEs']);	
} else if(isset($_POST['search_specialEs'])){
	$search_specialEs = $_POST['search_specialEs'];
	session_register('search_specialEs');
	$_SESSION['search_specialEs'] = $search_specialEs;
	session_unregister('search_special');
	unset($_SESSION['search_special']);
	session_unregister('search2_special');
	unset($_SESSION['search2_special']);	
...
$DEBUG=false;
$ppa = new PPA(1);
if(isset($_POST['send_special'])){
	$m = new Special($_POST['id'], $_POST['title'], $_POST['spanishTitle'],$_POST['gender'], $_POST['rated'],
		       $_POST['starring'],$_POST['description'], $_POST['ppa']);
...
//	echo "<script>if(window.opener)window.close();</script>";
}else if(isset($_POST['dup_special'])){
	$m = new Special(false, $_POST['title'], $_POST['spanishTitle'], $_POST['gender'], $_POST['rated'], 
		       $_POST['starring'],$_POST['description'],$_POST['ppa']);
	...
	$m->addChapter($c);
	$m->commit();
} else if (isset($_GET['delete_special']) && $_GET['delete_special']){
	$m = new Special($_GET['id']);
	$m->erase();
	unset($m);
}
?>
...
<table width="600" border="0" cellpadding="0" cellspacing="1">
  <?
if( (isset($_GET['add_special']) && $_GET['add_special']) || (isset($_GET['edit_special']) && $_GET['edit_special']) ){
	if(isset($_GET['edit_special']) && $_GET['edit_special']){
		$mv = new Special($_GET['id']);
		$cs = $mv->getChapters();
  ...
  <form name="f1" enctype="multipart/form-data" action="<?=$_SERVER['PHP_SELF']?>" method="post">
    ...
    <tr> 
      ...
      <td> <div align="left"> 
          <select name="gender" class="large">
            ...
            <option <?=($gender=="Teatro"?"selected":"")?> value="Teatro">Teatro</option>
   ...
   <tr> 
    ...
    <input type="hidden" name="id" value="<?=($_GET['add_special']?"0":$_GET['id'])?>">
    ...
    <tr> 
      <td height="39" colspan="3" align="center"><input type="submit" name="send_special" value="<?=($_GET['add_special']?"Crear":"Actualizar")?>" class="large">
        <br>
	<? if( $_GET['popup'] != 1 ){ ?>
        <input type="button" name="send2" value="Regresar" class="large" onClick='document.location="ppa.php?special=1"'>
	<? }?>
	  </td>
    </tr>
    <tr bgcolor="#CCCCCC"> 
	  ...
	  <? if (!$_GET['add_special']){
		  ?>Es posible dup_speciallicar los datos de esta Serie, y guardarlos como un registro nuevo.
		  <br><input type="submit" name="dup_special" value="Guardar como nuevo registro" class="large"><?
		 }
		 ?>
	  </td>
    </tr>
...
?>
	...
	<i><b>Nota:</b> puede usar el caracter '%' como comodín en las palabras de búsqueda.</i><br>
		<input type="text" class="large" name="search_special" value="<?=(isset($_SESSION['search_special'])?$_SESSION['search_special']:"")?>">
		<input type="submit" name="buscar" value="Buscar en Título">
	</form><br>
	<form name="f4" action="ppa.php" method="post">
		<input type="text" class="large" name="search_specialEs" value="<?=(isset($_SESSION['search_specialEs'])?$_SESSION['search_specialEs']:"")?>">
		<input type="submit" name="buscar" value="Buscar Título en Español">
	</form><br>		
	<form name="f3" action="<?=$_SERVER['PHP_SELF']?>" method="post">
		<input type="text" class="large" name="search2_special" value="<?=(isset($_SESSION['search2_special'])?$_SESSION['search2_special']:"")?>">
	...
	for($i=0; $i<count($letters); $i++){
		?><a href="ppa.php?search_special=<?=$letters[$i]?>%"><?=$letters[$i]?></a> - <?
	}
	if (isset($_SESSION['search_special']) || isset($_SESSION['search2_special']) || isset($_SESSION['search_specialEs'])){
		if (isset($_SESSION['search_special'])){
			$specials = $ppa->getSpecials($_SESSION['search_special']);
		} else {
			if (isset($_SESSION['search2_special'])){
				$specials = $ppa->getSpecialsByStarring($_SESSION['search2_special']);
			}else{
				if (isset($_SESSION['search_specialEs'])){
					$specials = $ppa->getSpecialsEs($_SESSION['search_specialEs']);
				}
			}
		}
		for ($i=0; $i<count($specials); $i++){
			?>
		  <tr> 
			<td bgcolor="#<?=($specials[$i]->getStarring()?"FFEEDD":"DDEEFF")?>"> <strong>
			<? if (isset($_SESSION['search_seriesEs'])){ ?>			 
			  <a href="ppa.php?edit_special=1&id=<?=$specials[$i]->getId()?>"><?=$specials[$i]->getSpanishtitle()?></a>
			  <? }else{ ?>
			  <a href="ppa.php?edit_special=1&id=<?=$specials[$i]->getId()?>"><?=$specials[$i]->getTitle()?></a>
			  <? } ?>			
			  </strong> </td>
			<td bgcolor="#FFFFFF">&nbsp;</td>
			<td align="center" bgcolor="#DDEEFF">
			  <a href="ppa.php?delete_special=1&id=<?=$specials[$i]->getId()?>">Borrar</a></td>
		  </tr>
	    <?
		}
		?>
	  ...
	  <tr> 
		<td colspan="3" align="center" bgcolor="#FFFFCC"><a href="ppa.php?add_special=1">Agregar 
		  Especial&nbsp;/&nbsp;Documental</a></td>
	  </tr><?
	}
}
?>
...
<tr>
      <td align="center" bgcolor="#000000"> <font color="#FFFFFF" size=2><br>
		...
		<input align="absmiddle" type=text name="for" class="large" value="<?=(isset($title)?$title:(isset($_SESSION['search_special'])?$_SESSION['search_special']:""))?>">&nbsp;
            <? if( isset( $_GET['popup'] ) || isset( $_POST['popup'] ) ){?>
