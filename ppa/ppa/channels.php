...
if(isset($_POST['send_channel'])){
	...
	$m->setReview($_POST["review"]);
	$m->setDefaultGmt($_POST["defaultGmt"]);
	$m->commit();
	$change_channel = true;
...
?>
      ...
      <tr>
        <td width="146"><div align="right"><strong>GMT predeterminado</strong></div></td>
        <td width="15">&nbsp;</td>
        <td width="439">
          <div align="left"><input class="large" type="text" name="defaultGmt" value="<?=$defaultGmt?>" maxlength="2">
        </div></td>
      </tr>
      <tr>
      ...
      <tr>
        <td><div align="right"><strong>Descripci&oacute;n larga</strong></div></td>
        <td>&nbsp;</td>
        <td>
          <div align="left">
            <textarea name="review" class="large" rows="3"><?=$review?></textarea>
        </div></td>
      </tr>
...
?>
	  ...
	  if( $_GET['dia'] != "" ){
		...
		$sql1 = "SELECT id, title, date, time, duration, new_episode FROM slot WHERE date =  '".$_GET['ano']."-".$_GET['mes']."-".$_GET['dia']."' AND channel = ".$channel->getId()." ORDER BY date, time";
	  }else{
...
//	  	$sql = "SELECT  s.id, s.date, s.time, s.duration, c.title, c.spanishTitle, c.id FROM slot s, slot_chapter sc, chapter c WHERE s.channel = ".$channel->getId()." AND s.date >= '".$_GET['ano']."-".$_GET['mes']."-01"."' AND s.date <= '".$_GET['ano']."-".$_GET['mes']."-".$numdays."' AND s.id = sc.slot AND sc.chapter = c.id ORDER BY s.date, s.time";
	  	$sql = "SELECT  s.id, s.date, s.time, s.duration, c.title, c.spanishTitle, c.id FROM slot s, slot_chapter sc, chapter c WHERE s.channel = ".$channel->getId()." AND s.date >= '".$_GET['ano']."-".sprintf("%02d", $_GET['mes'])."-00"."' AND s.date <= '".$_GET['ano']."-".sprintf("%02d", $_GET['mes'])."-".$numdays."' AND s.id = sc.slot AND sc.chapter = c.id ORDER BY s.date, s.time";
		$query = db_query( $sql );
		while( $row = db_fetch_array( $query ) ){
			$slot_array_chapter[$row[0]] = $row[6]." (|) ".$row[4]. " (|) ".$row[5];
		}
//		$sql1 = "SELECT id, title, date, time, duration FROM slot WHERE date >= '".$_GET['ano']."-".$_GET['mes']."-01"."' AND date <= '".$_GET['ano']."-".$_GET['mes']."-".$numdays."' AND channel = ".$channel->getId()." ORDER BY date, time";
		$sql1 = "SELECT id, title, date, time, duration, new_episode FROM slot WHERE date >= '".$_GET['ano']."-".sprintf("%02d", $_GET['mes'])."-01"."' AND date <= '".$_GET['ano']."-".sprintf("%02d", $_GET['mes'])."-".$numdays."' AND channel = ".$channel->getId()." ORDER BY date, time";
	  }
      $fecha_ant = "";
	  $query1 = db_query( $sql1 );
	  while( $row1 = db_fetch_array( $query1 ) ){
	  	$slot_id = $row1[0];
	  	$fecha = $row1['date'];
	  $new_episode=($row1['new_episode']==1 ? true : false);
		$hora = $row1['time'];
      ...
      <tr>
		...
		<font color="#217199">
		  ...
		  <? if( $chapterid != -1 ){ ?>
		   ...
		   <? }?>
          <strong><?=($new_episode?"*":"") . $slot_title?></strong> &nbsp;&nbsp;/&nbsp;&nbsp; <strong><font color="#FF9933"><?=$chapter_ortitle?></font></strong> &nbsp;&nbsp;/&nbsp;&nbsp; <strong><font color="#CC0000"><?=$chapter_estitle?></font></strong>
