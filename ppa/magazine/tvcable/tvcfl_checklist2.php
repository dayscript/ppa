...
<form name="f" method="post" action="?location=tvcfl_gxls&date=<?=$_GET['date']?>&id=<?=$_GET['id']?>&nohead=true">
...
<input type="button" onClick="tvcflBack();" name="send" value="<< Regresar ">
<input type="submit" name="send" value="Finalizar >>">
</form>
<form id="backform" method="post" action="?location=tvcfl_checklist&date=<?=$_GET['date']?>&id=<?=$_GET['id']?>" ><input type="hidden" name="ids" value="<?=implode(",", $_POST['specials'])?>" ></form>