...
$end_date    = mktime( 0, 0, 0, $date[1]+1, 2, $date[0] );

if( $_GET['group'] == 'premium'){
  $id_channels = "128, 129, 358, 127, 132, 133, 124, 125, 123";
}elseif( $_GET['group'] == 'cine'){
  $id_channels = "864, 116, 131, 108, 81, 68, 43";
}elseif( $_GET['group'] == 'series'){
  $id_channels = "311, 103, 446, 30";
}elseif( $_GET['group'] == 'culturales'){
  $id_channels = "95, 57, 505, 72, 327";
}elseif( $_GET['group'] == 'deportes'){
  $id_channels = "461, 780, 888, 77, 199";
}elseif( $_GET['group'] == 'internacionales'){
  $id_channels = "100, 114, 198, 63, 113";
  ...
  $id_channels = "79, 141, 59, 61, 50, 37, 84, 312, 337, 98, 866";
  $ROW_SIZE    = 56;
}elseif( $_GET['group'] == 'noticias'){
//  $id_channels = "193, 865, 55, 136, 799, 205";
  $id_channels = "193, 865, 55, 136, 799, 198";
  $ROW_SIZE    = 56;
}elseif( $_GET['group'] == 'religiosos'){
  $id_channels = "70, 64, 350";
  $ROW_SIZE    = 56;
}
...
$format_center[24] =& $workbook->addFormat( $format_array );
$format_center[24]->setSize(24);

$format_center[22] =& $workbook->addFormat( $format_array );
$format_center[22]->setSize(22);

$format_center[20] =& $workbook->addFormat( $format_array );
$format_center[20]->setSize(20);

$format_center[18] =& $workbook->addFormat( $format_array );
$format_center[18]->setSize(16);

$format_center[16] =& $workbook->addFormat( $format_array );
$format_center[16]->setSize(16);

$format_center[15] =& $workbook->addFormat( $format_array );
$format_center[15]->setSize(15);
...
$format_center[6] =& $workbook->addFormat( $format_array );
$format_center[6]->setSize(6);

$format_center[5] =& $workbook->addFormat( $format_array );
$format_center[5]->setSize(5);
...
//			$tmpTitle    = timeFormat( $tmpTime ) . " " . ($tmpDuration <=60 ? substr($time->getAttribute("title"),0,8) . "..." : $time->getAttribute("title") );
			$tmpGrouped  = $time->getAttribute("grouped");
			$tmpTitle    = timeFormat( $tmpTime ) . " " . ($tmpDuration <=60 ? substr($time->getAttribute("title"),0,16) . "..." : $time->getAttribute("title") );
			...
			{		
				...
				if(count(explode("\n",$title[$col]))<2 )
					if($tmpDuration <= 60)
						$title[$col] = $title[$col] == "" ? $tmpTitle : substr($title[$col],0,13) . "...\n" . substr($tmpTitle,0,13) . "...";
