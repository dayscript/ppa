...
function setNextButton()
{
	...
	{
		btn.value = "Finalizar >>";
		f.action  = "?location=ccfl_gwxml&date=<?=$_GET['date']?>&id=<?=$_GET['id']?>&nohead=true";
	}
	else
	{
		btn.value = "Siguiente >>";
		f.action = "?location=ccfl_chkl2&date=<?=$_GET['date']?>&id=<?=$_GET['id']?>";
	}
}
...
<form name="f" method="post" action="?location=ccfl_chkl2&date=<?=$_GET['date']?>&id=<?=$_GET['id']?>" >
