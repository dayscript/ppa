...
for( $j=1; $j<=30; $j+=1){ print ( "dia "      . $j . "\n" ); 
$url_site = "http://www.rtve.es/tve/programo/avan7/tv7s". ( $j <10 ? "0" : "" ) . $j ."09.htm";
...
//                      print_r($sitio->arSitio);
			...
			{
$hour = substr( $sitio->arSitio[$i], 0, 5 );

                                print($hour . "\t" );
                                print(substr($sitio->arSitio[$i], 5) . "\t" . $sitio->arSitio[++$i] ."\n");
			}
                }
        }
        else
