...
if( isset( $_POST['cambiar_x'] ) || isset( $_POST['aprobar_x'] ) ){
  $taquilla = new Taquilla( $_POST['id_obj'] );
  $contenido = $taquilla->getContenido();
  ...
  $taquilla->setContenido($contenido);
  $taquilla->commit();
  $contenido = $contenido->getTexto();
  $cambio = true;
}else{
   $sql = "select id from taquilla where pagina = ".$id;
   $query = db_query( $sql );
   $row = db_fetch_array( $query );
   $taquilla = new Taquilla( $row['id'] );
   $contenido = $taquilla->getContenido();
   $contenido = $contenido->getTexto();
}
if( trim( $contenido ) == "" ){
   $contenido = $taquilla->getBase();  
}

?>
<html>
<head>
<title>Intermedio - TAQUILLA</title>
...
<tr>
<td valign="top"><img src="../images/cabezote_taquilla.gif" width="400" height="40"></td>
</tr>
<tr>
<td>
<?
if(strstr($_SERVER['HTTP_USER_AGENT'],"MSIE")){
	...
	if(strstr($_SERVER['HTTP_USER_AGENT'],"Mozilla/5")  && !strstr( $_SERVER['HTTP_USER_AGENT'],"Macintosh" ) ){
	   ...
	   if( strstr( $_SERVER['HTTP_USER_AGENT'],"Macintosh" ) ){
	      echo $taquilla->contenido->getTexto();
	   }
	}
}
?>
...
<tr>
    <td align="center"> <textarea name="comentario" id="comentario" cols="50" rows="10" readonly="readonly"><?=$taquilla->contenido->getComentario()?></textarea>
	<br>
	</td>
</tr>
<tr>
...
<input type="hidden" name="id_obj" value="<?=$taquilla->getId()?>">
...
&nbsp;&nbsp;&nbsp;
<? if( $user == 'intercable' && $taquilla->contenido->getFechaAprobacion() == "0000-00-00" ){ ?>
