...
if( isset( $_POST['cambiar_x'] ) || isset( $_POST['aprobar_x'] ) ){
  $premium = new Premium( $_POST['id_obj'] );
  $contenido = $premium->getContenido();
  ...
  $premium->setContenido($contenido);
  $premium->commit();
  $contenido = $contenido->getTexto();
  $cambio = true;
}else{
   $sql = "select id from premium where pagina = ".$id;
   $query = db_query( $sql );
   $row = db_fetch_array( $query );
   $premium = new Premium( $row['id'] );
   $contenido = $premium->getContenido();
   $contenido = $contenido->getTexto();
}
if( trim( $contenido ) == "" ){
   $contenido = $premium->getBase();  
}

?>
<html>
<head>
<title>Intermedio - PREMIUM</title>
...
<tr>
<td valign="top"><img src="../images/cabezote_premium.gif" width="400" height="40"></td>
</tr>
<tr>
<td>
<?
if(strstr($_SERVER['HTTP_USER_AGENT'],"MSIE")){
	...
	if(strstr($_SERVER['HTTP_USER_AGENT'],"Mozilla/5")  && !strstr( $_SERVER['HTTP_USER_AGENT'],"Macintosh" ) ){
	   ...
	   if( strstr( $_SERVER['HTTP_USER_AGENT'],"Macintosh" ) ){
	      echo $premium->contenido->getTexto();
	   }
	}
}
?>
...
<tr>
    <td align="center"> <textarea name="comentario" id="comentario" cols="50" rows="10" readonly="readonly"><?=$premium->contenido->getComentario()?></textarea>
	<br>
	</td>
</tr>
<tr>
...
<input type="hidden" name="id_obj" value="<?=$premium->getId()?>">
...
&nbsp;&nbsp;&nbsp;
<? if( $user == 'intercable' && $premium->contenido->getFechaAprobacion() == "0000-00-00" ){ ?>
