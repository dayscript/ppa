...
if( isset( $_POST['cambiar_x'] ) || isset( $_POST['aprobar_x'] ) ){
  $destacados = new Destacados( $_POST['id_obj'] );
  $contenido = $destacados->getContenido();
  ...
  $destacados->setContenido($contenido);
  $destacados->commit();
  $contenido = $contenido->getTexto();
  $cambio = true;
}else{
   $sql = "select id from destacados where pagina = ".$id;
   $query = db_query( $sql );
   $row = db_fetch_array( $query );
   $destacados = new Destacados( $row['id'] );
   $contenido = $destacados->getContenido();
   $contenido = $contenido->getTexto();
}
if( trim( $contenido ) == "" ){
   $contenido = $destacados->getBase();  
}

?>
<html>
<head>
<title>Intermedio - DESTACADOS</title>
...
<tr>
<td valign="top"><img src="../images/cabezote_destacados.gif" width="400" height="40"></td>
</tr>
<tr>
<td>
<?
if(strstr($_SERVER['HTTP_USER_AGENT'],"MSIE")){
	...
	if(strstr($_SERVER['HTTP_USER_AGENT'],"Mozilla/5")  && !strstr( $_SERVER['HTTP_USER_AGENT'],"Macintosh" ) ){
	   ...
	   if( strstr( $_SERVER['HTTP_USER_AGENT'],"Macintosh" ) ){
	      echo $destacados->contenido->getTexto();
	   }
	}
}
?>
...
<tr>
    <td align="center"> <textarea name="comentario" id="comentario" cols="50" rows="10" readonly="readonly"><?=$destacados->contenido->getComentario()?></textarea>
	<br>
	</td>
</tr>
<tr>
...
<input type="hidden" name="id_obj" value="<?=$destacados->getId()?>">
...
&nbsp;&nbsp;&nbsp;
<? if( $user == 'intercable' && $destacados->contenido->getFechaAprobacion() == "0000-00-00" ){ ?>
