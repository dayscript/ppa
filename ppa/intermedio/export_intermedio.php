...
?>{\f1\fs17 <?=date("h:i", strtotime( $keys[$k] ))?>\tab }{\b\f1\fs17 <?=$keys1[$m]?>\tab}{\f1\fs17 <?=spanishString($t1)?>}{ \par }{\tab\tab}{\f1\fs17 <?=spanishString($t2)?>}{ \par }<?
		  }else{
			?>{\f1\fs17 <?=date("h:i", strtotime( $keys[$k] ))?>\tab }{\b\f1\fs17 <?=$keys1[$m]?>\tab}{\f1\fs17 <?=spanishString($slots_array[$date][$keys[$k]][$keys1[$m]])?>}{ \par }<?
																				       }
			}else{			  
			  if( strlen($slots_array[$date][$keys[$k]][$keys1[$m]]) > MAX_CHAR_LENGTH ){
			    $pos = strrpos(substr($slots_array[$date][$keys[$k]][$keys1[$m]],0,MAX_CHAR_LENGTH)," ");
