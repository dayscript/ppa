...
if( isset( $_POST['cambiar_x'] ) || isset( $_POST['aprobar_x'] ) ){
  $television = new Television( $_POST['id_obj'] );
  $contenido = $television->getContenido();
  ...
  $television->setContenido($contenido);
  $television->commit();
  $contenido = $contenido->getTexto();
  $cambio = true;
}else{
   $sql = "select id from television where pagina = ".$id;
   $query = db_query( $sql );
   $row = db_fetch_array( $query );
   $television = new Television( $row['id'] );
   $contenido = $television->getContenido();
   $contenido = $contenido->getTexto();
}
if( trim( $contenido ) == "" ){
   $contenido = $television->getBase();  
}

?>
<html>
<head>
<title>Intermedio - TELEVISION</title>
...
<tr>
<td valign="top"><img src="../images/cabezote_television.gif" width="400" height="40"></td>
</tr>
<tr>
<td>
<?
if(strstr($_SERVER['HTTP_USER_AGENT'],"MSIE")){
	...
	if(strstr($_SERVER['HTTP_USER_AGENT'],"Mozilla/5")  && !strstr( $_SERVER['HTTP_USER_AGENT'],"Macintosh" ) ){
	   ...
	   if( strstr( $_SERVER['HTTP_USER_AGENT'],"Macintosh" ) ){
	      echo $television->contenido->getTexto();
	   }
	}
}
?>
...
<tr>
    <td align="center"> <textarea name="comentario" id="comentario" cols="50" rows="10" readonly="readonly"><?=$television->contenido->getComentario()?></textarea>
	<br>
	</td>
</tr>
<tr>
...
<input type="hidden" name="id_obj" value="<?=$television->getId()?>">
...
&nbsp;&nbsp;&nbsp;
<? if( $user == 'intercable' && $television->contenido->getFechaAprobacion() == "0000-00-00" ){ ?>
