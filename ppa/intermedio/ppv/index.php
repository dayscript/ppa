...
if( isset( $_POST['cambiar_x'] ) || isset( $_POST['aprobar_x'] ) ){
  $ppv = new Ppv( $_POST['id_obj'] );
  $contenido = $ppv->getContenido();
  ...
  $ppv->setContenido($contenido);
  $ppv->commit();
  $contenido = $contenido->getTexto();
  $cambio = true;
}else{
   $sql = "select id from ppv where pagina = ".$id;
   $query = db_query( $sql );
...
if( trim( $contenido ) == "" ){
   $contenido = $ppv->getBase();  
}

?>
<html>
<head>
<title>Intermedio - PPV</title>
...
<tr>
<td valign="top"><img src="../images/cabezote_ppv.gif" width="400" height="40"></td>
</tr>
<tr>
<td>
<?
...
<tr>
    <td align="center"> <textarea name="comentario" id="comentario" cols="50" rows="10" readonly="readonly"><?=$ppv->contenido->getComentario()?></textarea>
	<br>
	</td>
</tr>
<tr>
...
<input type="hidden" name="id_obj" value="<?=$ppv->getId()?>">
...
&nbsp;&nbsp;&nbsp;
<? if( $user == 'intercable' && $ppv->contenido->getFechaAprobacion() == "0000-00-00" ){ ?>
