...
$IMAGEPATH = "http://200.71.33.249/ppa/chapter_images";
$ID_CLIENT = "65";
...
else if( $_GET['action'] == "highlight" && isset($_GET["id"]))
{
	$h = new Highlight();
...
else if( $_GET['action'] == "category" && isset($_GET["c"]))
{
	$db = new DataBase( DB_HOST, DB_USER, DB_PASS, DB_NAME, DB_DEBG );
	$q = isset($_GET['q']) ? ($_GET['q']>0 ? $_GET['q']:3):3;
	$d = isset($_GET['d']) ? (ereg("[0-9]{4}-[0-9]{2}", $_GET['d']) ? $_GET['d']:date("Y-m-d")):date("Y-m-d");
	...
	{
		case "special":
		$sql_p1 = "c.special > 0 AND ";
		break;
		case "serie":
		$sql_p1 = "c.serie > 0 AND ";
		break;
		case "movie":
		default:
		$sql_p1 = "c.movie > 0 AND ";
		break;
	}
	
	if($_GET["c"] == "children" ) //a very special case
	{
		$sql = "SELECT h.id_highlight, ch.name, s.title, s.time, s.date, c.description sdesc, c.id, c.points FROM " . 
