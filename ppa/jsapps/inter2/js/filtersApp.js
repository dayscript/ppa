...
GridApp.rechargeSelect = function (id) {
	...
	ch.id = id;
	if (id == "ppa_gender")    var _url = SERVICES_PATH + "?action=filters" + ((GridApp.header != 0) ? ("&header=" + GridApp.header) : ""); else    var _url = SERVICES_PATH + "?action=filters" + ((GridApp.category != 0) ? ("&category=" + GridApp.category) : "") + ((GridApp.header != 0) ? ("&header=" + GridApp.header) : "");
	...
	if (id == "ppa_gender")    var newsel = GridApp.createGenderSel(root_el.getElementsByTagName("categories").item(0)); else    var newsel = GridApp.createChannelSel(root_el.getElementsByTagName("channels").item(0));
...
GridApp.createStartDateField = function () {
	var div = document.createElement("div");
	var subDiv = GridApp.createField("Fecha Inicio");
	...
	h = (h > 9) ? h : "0" + h;
	if (d.getMinutes() > 30) var mins = "30"; else var mins = "00";
	if (GridApp.startDate != "") i.value = GridApp.startDate; else i.value = dt + "/" + m + "/" + d.getFullYear().toString().substr(2, 2);
	i.readOnly = "readonly";
	div.appendChild(i);
	var sd = new Date();
	startCal.setDate(sd);
...
GridApp.createEndDateField = function () {
	var div = GridApp.createField("Fecha Fin");
	var i = GridApp.createCalendar("ppa_endDate", endCal);
	i.onclick = GridApp.showEndCal;
	if (GridApp.endDate != "") i.value = GridApp.endDate; else i.value = "Calendario";
	i.readOnly = true;
	div.appendChild(i);
	//div.appendChild(GridApp.createField(""));
...
GridApp.createSearchButton = function () {
	...
	b.type = "submit";
	b.value = "Buscar";
	/*b.onclick = function(){try{ppaG.consultGrid(0);}catch(e){alert("La instancia principal no existe, revise la documentación! " + e)}};*/
	div.appendChild(b);
	return div;
};
...
GridApp.createFilters = function () {
	...
	fc.appendChild(ln);
	/*date*/
	fs = GridApp.createFiltersLine(GridApp.createStartDateField());
	fs.appendChild(GridApp.createInputField("Título", "ppa_title", GridApp.title));
	fc.appendChild(fs);

	fs = GridApp.createFiltersLine(GridApp.createEndDateField());
	fs.appendChild(GridApp.createInputField("Actor/Director", "ppa_actor", GridApp.actor));
	fc.appendChild(fs);

	ln = document.createElement( 'div' );
	ln.className = 'searchBtn';
